...
class WeightedMovingFilter:
    ...
    def reset(self, value):
        """Seed the complete filter window with one finite sample."""
        value = np.asarray(value, dtype=float).reshape(-1)
        if value.size != self._data_size:
            raise ValueError(
                f"Expected {self._data_size} values, got {value.size}"
            )
        if not np.all(np.isfinite(value)):
            raise ValueError("Filter reset value must contain only finite values")
        self._data_queue = [value.copy() for _ in range(self._window_size)]
