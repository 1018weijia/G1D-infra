...
### 对照实验：不回退直接接管（分支 `feat/rlt-stage2-no-rollback`）

启动命令前加 `NO_ROLLBACK=1`（传给 `policy_deploy.py --no-rollback`）。按 `B` 时中断当前块、给服务端发的回退标注（`rewind`）、清掉排队的奖励键和 `Q` 标记，都和回退接管完全一样；唯一区别是手臂不沿原路倒放，停在被打断的位置，直接进入对齐，手柄 `A` 接管。接管中按 `B` 同样只是原地停下重新对齐。

两组数据要分开训：对照组用另开的服务端进程（不同端口、不同 `--save-dir`、不带 `--resume` 或从同一份起点状态复制出来 resume），机器人上用另一个本地端口的隧道，不要连到正在跑的回退组服务端。
