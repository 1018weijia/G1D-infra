...
| `P` | 当前这一拍 +0.5，回合继续 |
| `Y` | 当前这一拍 +1，本块结束后成功收束，发 `episode_end` |
| `N` | 本块结束后失败收束（奖励保持已按下的分数），发 `episode_end` |
| `A` | 对齐完成后接管。人工关节记成 `intervention=true`。再按 `A` 或 `S` 交回策略 |
| `B` | 物理回退。已执行的策略段先按原动作入库，再发 `rewind_exit`：坏分支最后一步奖励 -0.2，切断 bootstrap。没有可回放的历史时发 `rewind_credit`，并给前一段末步 +0.1 |
| `Q` | 退出 |

`Y` / `N` / `P` 只在策略正在执行时有效。奖励写在按下的那一拍，不是固定写在块的最后一拍。
