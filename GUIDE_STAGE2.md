...
| `A` | 对齐完成后接管。人工关节记成 `intervention=true`。对齐等待、交接过渡和接管后原地不动的拍都不记；和 Franka 的死区一样，任一只手末端动 1.5 mm、转 0.01 rad、关节变 0.02 rad 或夹爪明显变化才算一步，接管块的观测也在第一次真正动的时候拍。再按 `A` 或 `S` 时手臂先停在当前姿势，等服务端空闲再要下一块 |
