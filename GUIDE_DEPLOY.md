...
已经从 GPU 打好反向隧道时，用 `SKIP_TUNNEL=1`。这时客户端连机器人本机 `127.0.0.1:16555`（`RL_LOCAL_PORT` 可改），不再占用 `15555`。`15555` 仍是原来那条部署隧道的本地口，对端是另一台 GPU 的 `5555`。在线服务只绑训练机自己的 `127.0.0.1:5555`，反向隧道示例：
