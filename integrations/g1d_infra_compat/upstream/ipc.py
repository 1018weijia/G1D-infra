...
4) stop the current episode and mark it failed
    {
        "reqid": unique id,
        "cmd": "CMD_RECORD_FAIL"
    }
...
- Heartbeat Pub format:
    {
        ...
        "RECORD_TOGGLE": True | False,  # pending start/stop request
        "PAUSED": True | False,         # teleop holding pose for scene reset
    }
"""
...
class IPC_Server:
    ...
    cmd_map = {
        "CMD_START": "r",          # launch, then pause/resume teleop
        "CMD_STOP": "q",           # exit
        "CMD_RECORD_TOGGLE": "s",  # start & stop (toggle record)
        "CMD_RECORD_FAIL": "f",    # stop record and mark episode failed
    }
...
if __name__ == "__main__":
    ...
    def on_press(key: str):
        ...
        if key == "r":
            logger_mp.info("▶️ Sending start/pause/resume command...")
            ...
            logger_mp.info("❌ Sending record-fail command...")
            rep = client.send_data("CMD_RECORD_FAIL")
            logger_mp.info("Reply: %s", rep) 
    ...
    logger_mp.info("✅ Client started, waiting for keyboard input:\n [r] start/pause/resume, [s] start/stop record, [f] fail-stop record, [b] heartbeat, [q] exit")
    try:
        while True:
            time.sleep(1.0)
